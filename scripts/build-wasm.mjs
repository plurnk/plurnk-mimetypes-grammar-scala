#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-scala.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-scala-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-scala.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as scala.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "scala.wasm"));
const bytes = (await readFile(path.join(repoRoot, "scala.wasm"))).length;
console.log(`scala.wasm: ${bytes} bytes (built from ${pin})`);
