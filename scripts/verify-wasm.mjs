#!/usr/bin/env node
// CI byte-identical reproducibility check for scala.wasm.
...
const committed = await readFile(path.join(repoRoot, "scala.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed scala.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-scala-verify-"));
...
console.log(`rebuilt scala.wasm sha256: ${rebuiltHash}`);
